...
public class EventTests : IClassFixture<IntegrationTestEnvironment>
{
    ...
    [InlineData(TestScenario.IdentityProviderUserIdentifierReader, true, new[] { "tev", "ooe", "fev" }, new[] { UserInEventRole.Writer, UserInEventRole.Reader, (UserInEventRole)0 },
                new[] { false, false, true })]
    public async Task GetEventsOfUser(string userIdentifier,
                                      bool includeRequestedEvents,
                                      string[] expectedEventAcronyms,
                                      UserInEventRole[] expectedRoles,
                                      bool[] expectedRequestSent)
    {
        ...
        {
            events.ShouldContain(evt => evt.Role == expectedRoles[i]);
        }
    }
    ...
    public async Task SearchEvents(string searchString,
                                   bool includeRequestedEvents,
                                   bool includeAuthorizedEvents,
                                   string[] expectedSearchResultAcronyms)
    {
