...
namespace EventRegistrator.Functions.Registrations.Cancellation
{
    public static class CancelRegistration
    {
            ...
            TraceWriter log)
        {
            ...
            var preventPromotion = req.GetQueryNameValuePairs().FirstOrDefault(kvp => string.Compare(kvp.Key, "preventPromotion", StringComparison.OrdinalIgnoreCase) == 0).Value == "true";
            ...
            using (var dbContext = new EventRegistratorDbContext())
            {
                ...
                {
                    CancelSpot(place, registrationId);
                }                
                ...
                if (registration.RegistrationForm.EventId.HasValue && !preventPromotion)
                {
                    foreach (var place in places)
                    {
                        await ServiceBusClient.SendEvent(new TryPromoteFromWaitingListCommand { RegistrableId = place.RegistrableId },
                                                         TryPromoteFromWaitingList.TryPromoteFromWaitingListQueueName);
                    }
                }

                await ServiceBusClient.SendEvent(new ComposeAndSendMailCommand { RegistrationId = registrationId, Withhold = true },
        ...
        public static void CancelSpot(Seat place, Guid registrationId)
        {
