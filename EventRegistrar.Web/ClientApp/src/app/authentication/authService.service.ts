...
export class AuthService {
  constructor(private http: HttpClient) {
    this.http.get<Ticket[]>("https://eventregistrar.azurewebsites.net/.auth/me").subscribe(ticket => {
      try {
        const firstName = ticket[0].user_claims.find(c => c.typ === "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname").val;
        const lastName = ticket[0].user_claims.find(c => c.typ === "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/surname").val;
