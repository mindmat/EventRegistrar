    ...
    public class SingleRegistrationProcessor
    {
        ...
        public async Task<IEnumerable<Seat>> Process(Registration registration)
        {
                            ...
                            case MappingType.SingleRegistrable:
                                {
                                    ...
                                    {
                                        var spot = await _spotManager.ReserveSingleSpot(registration.EventId, questionOptionMapping.RegistrableId.Value, registration.Id, true);
                                        if (spot == null)
                                        {
                            ...
                            case MappingType.PartnerRegistrableLeader:
                                {
                                    if (questionOptionMapping.RegistrableId != null)
                                    {
                                        ...
                                        {
                                            RegistrableId = questionOptionMapping.RegistrableId.Value,
                                            Role = Role.Leader
                                        });
                                    }
                                    break;
            ...
            if (requestsWithoutRole.Any())
            {
                if (defaultRole == null)
                {
                    throw new InvalidOperationException($"Invalid mapping configuration: Mappings to partner registrable {requestsWithoutRole.Select(rwr => rwr.RegistrableId.ToString()).StringJoin()} but no role defined");
                }
            ...
            if (soldOutRegistrableIds.Any())
            {
                var soldOutRegistrableNames = await _registrables.Where(rbl => soldOutRegistrableIds.Contains(rbl.Id))
                                                                 .Select(rbl => rbl.Name)
                                                                 .ToListAsync();
                registration.SoldOutMessage = soldOutRegistrableNames.Select(rbn => string.Format(Properties.Resources.RegistrableSoldOut, rbn))
