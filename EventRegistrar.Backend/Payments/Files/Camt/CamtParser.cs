    ...
    public class CamtParser
    {
        ...
        public CamtFile Parse(XDocument xml)
        {
            ...
            var entries = statement.Descendants(ns + "Ntry").Select(ntry => new CamtEntry
            {
                ...
                Message = ntry.Descendants(ns + "NtryDtls").Descendants(ns + "TxDtls").Descendants(ns + "RmtInf")?.Descendants(ns + "Ustrd")?.Select(nod => nod.Value)?.StringJoin(Environment.NewLine),
                ...
                Charges = ntry.Descendants(ns + "Chrgs").Descendants(ns + "TtlChrgsAndTaxAmt").FirstOrDefault()?.Value.TryToDecimal(),
