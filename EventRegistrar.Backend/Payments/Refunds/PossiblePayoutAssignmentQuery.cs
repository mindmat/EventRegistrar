...
namespace EventRegistrar.Backend.Payments.Refunds
{
    ...
    public class PossiblePayoutAssignmentQueryHandler : IRequestHandler<PossiblePayoutAssignmentQuery, IEnumerable<PossiblePayoutAssignment>>
    {
        ...
        public async Task<IEnumerable<PossiblePayoutAssignment>> Handle(PossiblePayoutAssignmentQuery query, CancellationToken cancellationToken)
        {
            ...
            var payouts = await _payoutRequests.Where(por => por.Registration.EventId == query.EventId
                                               ...
                                               .Select(por => new PossiblePayoutAssignment
                                               {
                                                   ...
                                                   Ibans = por.Registration.Payments.Select(pmt => pmt.Payment.DebitorIban).Where(ibn => ibn != null).ToArray()
                                               })
        ...
        {
            var participantParts = payoutRequestCandidate.Participant?.Split(new[] { ' ', '-' }, StringSplitOptions.RemoveEmptyEntries)?.Select(wrd => wrd.ToLowerInvariant()) ?? new List<string>();

            var wordsInOpenPayment = openPayment.Info.Split(' ').Select(nmw => nmw.ToLowerInvariant()).ToList();

            return payoutRequestCandidate.Ibans.Contains(openPayment.CreditorIban) ? 50 : 0
