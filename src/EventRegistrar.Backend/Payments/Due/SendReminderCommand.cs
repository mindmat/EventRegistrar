...
public class SendReminderCommandHandler : IRequestHandler<SendReminderCommand>
{
    ...
    public async Task<Unit> Handle(SendReminderCommand command, CancellationToken cancellationToken)
    {
                                      ...
                                      .Select(reg => new
                                                     {
                                                         ...
                                                         StartPaymentPeriodMail = reg.Mails
                                                             .Where(map =>
                                                                 ...
                                                                 _paymentConfiguration.MailTypes_Accepted.Contains(
                                                                     map.Mail.Type.Value))
                                                             .Select(map => map.Mail)
        ...
        if (registration.IsWaitingList == true)
        {
            _logger.LogInformation(
                $"Registration {command.RegistrationId} is on the waiting list and doesn't have to pay yet");
            return Unit.Value;
        }
    ...
    {
        return (DateTime.UtcNow - startOfGracePeriodUtc).TotalDays >
               (paymentGracePeriod ?? _paymentConfiguration.PaymentGracePeriod);
    }
}