    ...
    IEnumerable<PossibleRepaymentAssignment>>
{
    ...
    private static int CalculateMatchScore(PossibleRepaymentAssignment paymentCandidate, ReceivedPayment openPayment)
    {
        ...
        var wordsInCandidate =
            paymentCandidate.Info?.Split(new[] { ' ', '-' }, StringSplitOptions.RemoveEmptyEntries)
                            ?.Select(wrd => wrd.ToLowerInvariant())
                            ?.ToList() ?? new List<string>();

        var unsettledAmountInOpenPayment = openPayment.Amount -
                                           openPayment.Assignments.Sum(asn =>
                                               asn.PayoutRequestId == null ? asn.Amount : -asn.Amount);
        var wordsInOpenPayment = openPayment.Info.Split(new[] { ' ', '-' }, StringSplitOptions.RemoveEmptyEntries)
                                            .Select(wrd => wrd.ToLowerInvariant())
                                            .ToHashSet();
