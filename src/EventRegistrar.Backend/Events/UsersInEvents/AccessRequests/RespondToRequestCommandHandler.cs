...
public class RespondToRequestCommandHandler : IRequestHandler<RespondToRequestCommand>
{
    ...
    {
        var request =
            await _accessRequests.FirstAsync(req => req.Id == command.AccessToEventRequestId, cancellationToken);
        if (request.Response.HasValue) throw new ArgumentException("Request has already been answered");
    ...
    {
        if (request.UserId_Requestor.HasValue) return request.UserId_Requestor.Value;
