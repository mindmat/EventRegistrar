        ...
        if (request.IncludeRequestedEvents)
                ...
                _authenticatedUser != AuthenticatedUser.None)
            {
                var requestedEvents = await _accessRequests.WhereIf(_authenticatedUserId.UserId.HasValue,
                                                               req => req.UserId_Requestor ==
                                                                      _authenticatedUserId.UserId.Value)
                                                           .WhereIf(
                                                               !_authenticatedUserId.UserId.HasValue &&
                                                               _authenticatedUser != AuthenticatedUser.None,
                                                               req => req.IdentityProvider ==
                                                                      _authenticatedUser.IdentityProvider
                                                                   && req.Identifier == _authenticatedUser
