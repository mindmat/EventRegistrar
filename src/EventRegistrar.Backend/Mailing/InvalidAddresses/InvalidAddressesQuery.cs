...
public class InvalidAddress
{
    ...
    public DateTimeOffset? LastMailSent { get; set; }
...
public class InvalidAddressesQueryHandler(IQueryable<Registration> registrations) : IRequestHandler<InvalidAddressesQuery, IEnumerable<InvalidAddress>>
{
    ...
    {
        var invalidMails = (await registrations.Where(reg => reg.EventId == query.EventId
                                                          && reg.State != RegistrationState.Cancelled
                                                          && reg.Mails.Any(map => map.State != null)
                                                          && !reg.Mails.Any(map =>
                                                                                map.State != null && DeliveredMailStates.Contains(map.State)))
                                               .Select(reg => new
                                                              {
        ...
        {
            var otherRegistrations = await registrations.Where(reg => reg.Id != invalidMail.RegistrationId
