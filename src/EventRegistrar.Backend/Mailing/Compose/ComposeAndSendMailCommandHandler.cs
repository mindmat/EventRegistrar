...
public class ComposeAndSendMailCommandHandler : IRequestHandler<ComposeAndSendMailCommand>
{
    ...
    public async Task<Unit> Handle(ComposeAndSendMailCommand command, CancellationToken cancellationToken)
    {
        ...
        foreach (var mailToRegistration in mappings.Select(reg =>
                     new MailToRegistration { Id = Guid.NewGuid(), MailId = mail.Id, RegistrationId = reg.Id }))
        ...
        var sendMailCommand = new SendMailCommand
                              {
                                  ...
                                  To = mappings.Select(reg =>
                                                   new EmailAddress
                                                   { Email = reg.RespondentEmail, Name = reg.RespondentFirstName })
                                               .ToList()
                              };
