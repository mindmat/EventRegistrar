...
public class
    RegistrationsOnWaitingListQueryHandler : IRequestHandler<RegistrationsOnWaitingListQuery,
        IEnumerable<WaitingListSpot>>
{
    ...
    public async Task<IEnumerable<WaitingListSpot>> Handle(RegistrationsOnWaitingListQuery query,
                                                           CancellationToken cancellationToken)
    {
        return await _seats.Where(spt => spt.Registrable.EventId == query.EventId
                           ...
                           .Select(spt => new WaitingListSpot
                                          {
                                              Leader = spt.RegistrationId == null
                                                  ? null
                                                  : new WaitingListRegistration
                                                    {
                                                        ...
                                                        OptionsSent = spt.Registration.Mails.Any(map =>
                                                                          map.Mail.Type ==
                                                                          MailType.OptionsForRegistrationsOnWaitingList
                                                                       && !map.Mail.Discarded)
                                                                   || spt.Registration.FallbackToPartyPass == true
                                                    },
                                              Follower = spt.RegistrationId_Follower == null
                                                  ? null
                                                  : new WaitingListRegistration
                                                    {
                                                        ...
                                                        OptionsSent = spt.Registration_Follower.Mails.Any(map =>
                                                                          map.Mail.Type ==
                                                                          MailType.OptionsForRegistrationsOnWaitingList
                                                                       && !map.Mail.Discarded)
                                              ...
                                              PlaceholderPartner = spt.IsPartnerSpot && (spt.RegistrationId == null ||
                                                  spt.RegistrationId_Follower == null)
                                                  ? spt.PartnerEmail
                                                  : null,
                                              IsOnWaitingList = spt.IsWaitingList || spt.Registration != null &&
                                                  spt.Registration.IsWaitingList == true,
